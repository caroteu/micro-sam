...
def main():
    ...
    vit_b = pd.read_csv(f"{EXPERIMENT_ROOT}benchmark_final/benchmark_vit_b_gpu.csv")    
    vit_l = pd.read_csv(f"{EXPERIMENT_ROOT}benchmark_final/benchmark_vit_l_gpu.csv")    
    vit_h = pd.read_csv(f"{EXPERIMENT_ROOT}benchmark_final/benchmark_vit_h_gpu.csv")    

    #vit_t_cpu = pd.read_csv(f"{EXPERIMENT_ROOT}benchmark_vit_t_cpu.csv")    
    vit_b_fast = pd.read_csv(f"{EXPERIMENT_ROOT}benchmark_vit_b_ft_fast.csv")    
    vit_l_fast = pd.read_csv(f"{EXPERIMENT_ROOT}benchmark_vit_l_ft_fast.csv")    
    vit_h_fast = pd.read_csv(f"{EXPERIMENT_ROOT}benchmark_vit_h_ft_fast.csv")   

    fig, axs = plt.subplots(1, 3, figsize=(14, 6), subplot_kw=dict(polar=True))

    # The order of the models matters
    get_radar_plot(axs[0], [vit_b, vit_b_fast], "ViT-B")
    get_radar_plot(axs[1], [vit_l, vit_l_fast], "ViT-L")
    get_radar_plot(axs[2], [vit_h, vit_h_fast], "ViT-H") 
